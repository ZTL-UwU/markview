...
图片下方默认显示 title 作为 caption；没有 title 时使用 alt。MVSS 的 `[img.caption]` 可调整来源、字号、颜色、对齐和间距，`source = "none"` 可关闭。图注和加载占位文字都可以逐字选择、复制。
...
上面的图片不存在，占位框会显示替代文本与错误原因。图片路径相对文档所在目录解析，也支持 `file:`、`http(s):` 和 `data:` 地址；`--offline` 会拒绝网络图片。
